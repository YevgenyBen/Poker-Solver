...
- **THE ENGINE CANNOT BET LESS THAN 2.5x THE POT, and it costs +0.0863 bb
  ...
  **SHIPPED AND BENCHMARKED (M206-M208).** `FLOP_RAISE_SIZES =
  ((0.33, 0.75, 2.5), 3.0, 2.2)`. Action paths can name a size
  (`"raise:12.50"`); a bare kind still means `BARE_RAISE_MEANS_MULTIPLE`
  (2.5x), so none of the 59 suite call sites or 18 frontend ones changed
  meaning, and ambiguity past a street's opening decision is a 422 rather
  than a guess.
  **Paired benchmark, 12 sessions / 3,193 decisions**: p50 **0.468 ->
  0.476s** (unchanged), p90 **2.03 -> 2.99s**, max 3.30s, **zero
  decisions over 5s and zero defects in both arms**. The whole cost is
  in the tail.
  **Facing a bet in the costly band (n=97)**, where M188/M189 put 74% of
  all cost: **+0.4869 -> +0.2529 bb, paired -0.2340 +/- 0.1007 = 2.32
  sigma**, over-1bb decisions **26 -> 14**, worst **6.39 -> 3.19 bb**.
  Mechanism is F38's axis — against the reference's 0.4878 fold
  frequency the old arm over-folds by **+0.1111** and the menu by
  **+0.0556**, halved.
  **Caveats**: split-half replicates in direction but not tightly in
  magnitude (-0.3158 / -0.1537); per spot it is near a coin flip (50/46)
  so the gain is the tail.
  **M209 closed the small-bet blind spot, and it was the biggest error in
  this project's accuracy work.** A player facing a half-pot bet could
  not say so, so `["raise"]` was modelled as a 2.5x overbet. 60 spots:
  | bet faced | ref folds | menu folds | OLD folds | menu bb | OLD bb | sigma |
  |---|---|---|---|---|---|---|
  | 0.33x | 0.0000 | 0.0002 | **0.6340** | +0.0071 | **+1.7400** | 10.88 |
  | 0.75x | 0.0769 | 0.0591 | **0.6340** | -0.0507 | **+1.6914** | 9.62 |
  | 2.50x | 0.5273 | 0.5841 | 0.6340 | +0.1701 | +0.4425 | 2.13 |
  The old arm's fold frequency is IDENTICAL on all three rows because it
  always read its 2.5x node. Against a third-pot bet the correct fold is
  **zero** and it folded **63%** — **1.74 bb on one decision**. It is a
  CAPABILITY gap, not an accuracy one: the old engine could not be asked
  the question. Old-arm rows are mapped by KIND (mean remapped mass
  0.0412, so the figure barely depends on it; the fold axis needs no
...
- **The TURN is solved as its OWN street, not chained from the flop
  ...
  of 4 classes keeping a median **4.6% of the opponent's range mass**,
  against the flop's 95% after M172), **57.7% of all postflop advice**,
  and never validated against a reference.
  Chaining is what made it thin - `solve_flop_turn` solves the flop AND
  the turn through chance nodes, so cost explodes with width: cap 8 is
  3.3x, cap 14 8.8x, **cap 26 40x (66 seconds)**. `solve_flop` already
  solves ONE street with runouts averaged at the terminal, and a
  four-card board is resolved EXACTLY (`remaining_needed == 1`, M154), so
  the turn that way is **47x cheaper at equal coverage**.
  10 turn spots from real play against a full-range standalone reference:
  mean error **0.3601 -> 0.1361**, spots over 0.10 error **7/10 -> 3/10**,
  solve 1.67s -> 0.28s. Non-monotone past cap 26 (M141's conservation law
  again, not the flop's clean threshold) - but the shipped setting was the
  worst arm tested.
  Validated over three 120-hand sessions, 837 decisions: **turn median
  7.94s -> 0.87s (9.1x), max 9.68s -> 1.41s**, decisions over 8s 9.3% ->
  3.1%, zero defects, zero uniform rows, flop and river unchanged.
  **The isolated solve figure predicted 1.2x end to end and the real gain
  is 9.1x** - the chained request was also paying for a flop solve that
  standalone skips. Isolated arms and requests do not measure the same
  work, in both directions (M170 and M172 each UNDER-quoted).
  **The flop tree is built but never solved**: the terminal's pot, who
  folded and each player's investment are structural, and
  `build_street_tree` builds children lazily, so walking one path
  materialises only that path.
  **What is NOT established: whether standalone is RIGHT versus chained**,
  as opposed to better-converged. The reference is itself standalone, and
  the comparison cannot be run - at the only coverage where chained is
  affordable (cap 14) both arms are unstable, agreeing on 6 of 8 spots and
  disagreeing on 2 in OPPOSITE directions. What standalone gives up is the
  flop betting round's influence on the turn. `TURN_SOLVE_STANDALONE =
  False` restores the chained path exactly; the flag exists because that
  question is open, not as a migration aid.
  Turn strategies got more decisive and it is convergence, not collapse:
  mixed (top action < 0.9) falls 37.3% -> 22.1%, but the genuinely
  indifferent band (0.40-0.60) is UNCHANGED (7.8% -> 8.6%) while the
  half-resolved middle (0.60-0.90) drops 28.5% -> 13.6%. A degenerate
  solve flattens the indifferent spots too.
  Guarded by `test_the_standalone_turn_solves_at_the_wider_coverage`,
  which exists because mutation testing showed reverting the cap to 4
  broke **nothing** - every other turn test checks structure, none checked
  coverage.
...
**Serialise the river, or the control fails on spots where the river
matters (M257 F62/F67).** A two-round dump values a turn leaf at
`equity * pot` while the strategy being scored was optimised against
river play the solver ran and did not serialise. Same spot, nothing else
changed: two rounds **8.4x FAIL**, three rounds **2.5x PASS**; at SPR
6.17 with matched menus **0.8x PASS**, below the bound as theory
requires. (The 8.4x was first read as "the walk overstates" in general -
it was one spot; four of six passed at two rounds.) Three rounds cost
**633 MB at SPR 2.53 and 3.47 GB at SPR 6.17**, and a resident dump
takes **~4.5x its file size in RAM**, so one at a time.

**Give the reference OUR bet menu, read off a real response (M257
F64).** It was handed `(33, 75)` while `/advise` offers 33/75/**250**
plus all-in; at SPR 2.53 the 250% bet IS the all-in and nothing shows,
at SPR 6.17 our overbet was scored as a 75%-pot bet. Matching is free.
**And compare sizes with a tolerance** (`SIZE_MATCH_TOLERANCE`, 5%
relative): the solver rounds amounts to integers, and an exact
comparison counted rounding as remapping (F65).

**What REGRET can and cannot measure (M258).** `Walk.regret_of_row` is
`max_a Q(a) - EV(ours)`, bounded to actions the range plays at least
`MIN_ACTION_SUPPORT` (1%), with both arms on one support.
- **Unbounded, it maximises over untrained subtrees**: a flop root the
  reference checks 100% priced at 2.57% of pot, 0.35% bounded.
- **It cannot compare cells of different sizes** - the bias grows with
  how much untrained subtree a cell's max can reach. M257's "facing a
  bet is 38x the opening decision" was withdrawn for this.
- **It cannot express disagreement at a PURE reference node.** On one
  flop 98.8% of our row sat on actions the reference never plays and
  regret read 0.0000. **Report off-support mass beside every regret**;
  at a pure node the only honest measure is the frequency question.
- **The support threshold can carry the answer**: one turn cell moved
  **43x** between 0.01 and 0.10, so a figure from it is an UPPER bound,
  not a point estimate.
- **Sound for the SAME cell across arms**, where the bias is common.
